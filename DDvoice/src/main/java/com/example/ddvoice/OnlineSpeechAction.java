...
public class OnlineSpeechAction implements AdapterView.OnItemClickListener,View.OnClickListener {
    ...
    public void startSpeenchRecognition(){//???????
        ...
        if (ret != ErrorCode.SUCCESS) {
            Log.d(TAG, "" + ret);
            showTip("??Ð´??????????" + ret);
            //info.makeText(getApplicationContext(), "??Ð´???,??????" + ret, 100).show();
        }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data){//??Ð´onActivityResult
        if(requestCode == 0){
        ...
        public void onInit(int code) {
            ...
            if (code != ErrorCode.SUCCESS) {
               showTip("???????????????"+code);
                ...
                // ???????????????????startSpeaking????
                // ????Ðµ????????onCreate?????Ð´?????????????????????startSpeaking???Ðº???
                // ????????????onCreate?Ðµ?startSpeaking????????????
            }
        }
    };
